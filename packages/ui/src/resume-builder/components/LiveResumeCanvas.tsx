...
export function LiveResumeCanvas(props: LiveResumeCanvasProps) {
  ...
  return (
    ...
    >
      {/* DOCUMENT-CENTERED LAYOUT: The resume is the primary visual object.
       * Centered with generous margins so it dominates the viewport.
       * The document panel uses a slightly elevated surface with shadow
       * and border to create the unmistakable impression of a real resume
       * document sitting on the workspace. */}
      <div className="max-w-[820px] mx-auto px-10 py-8">
        ...
        >
          ...
          <CanvasSectionWrapper
            ...
            onAnchorRegister={props.onAnchorRegister}
            onSectionAction={handleSectionAction}
            testId="canvas-section-experience"
          >
            <ExperienceCanvasSection
              experience={props.draft.experience}
              editMode={props.editMode}
              isSelected={props.selectedSectionId === 'experience'}
              editingField={editingField}
              editingText={editingText}
          ...
          <CanvasSectionWrapper
            ...
            onAnchorRegister={props.onAnchorRegister}
            onSectionAction={handleSectionAction}
            testId="canvas-section-education"
          >
          ...
          {/* ---- Certifications ---- */}
          {/* CANONICAL ORDER: Certifications appears before Skills per the
           * federal-section-meta displayOrder (certifications=4, skills=5).
           * This ensures the document matches the left rail order. */}
          <CanvasSectionWrapper
            ...
            onAnchorRegister={props.onAnchorRegister}
            onSectionAction={handleSectionAction}
            testId="canvas-section-certifications"
          >
          ...
          <CanvasSectionWrapper
            ...
            onAnchorRegister={props.onAnchorRegister}
            onSectionAction={handleSectionAction}
            testId="canvas-section-federal-details"
          >
          ...
          {/* ================================================================
           * DOCUMENT PAGE BREAK INDICATOR — Visual separator when content
           * exceeds one page. Uses a DOM measurement approach: a ref on
           * the document panel measures rendered height. If the content
           * exceeds one letter-size page height (~1056px at 96dpi), a
           * visual page-break marker appears between pages. This is the
           * foundational page-flow behavior — content naturally continues
           * onto page 2 as it grows, with a clear visual signal.
           * ================================================================ */}
          <PageBreakIndicator documentPanelRef={props.documentPanelRef} />
          ...
          <CanvasSectionWrapper
            ...
            onAnchorRegister={props.onAnchorRegister}
            onSectionAction={handleSectionAction}
            testId="canvas-section-supporting-evidence"
          >
...
function CanvasSectionWrapper(props: {
  ...
  }, [props.sectionId]); /* eslint-disable-line react-hooks/exhaustive-deps -- registration only needed once per section mount */
...
function ActionChip(props: {
  ...
  /* Compute background intensity based on interaction state.
   * Pressed wins over hover — pressed is the strongest visual. */
  let chipBg = 'color-mix(in srgb, var(--p-accent) 8%, transparent)';
  let chipBorder = '1px solid color-mix(in srgb, var(--p-accent) 20%, transparent)';
  if (isPressed) {
    chipBg = 'color-mix(in srgb, var(--p-accent) 18%, transparent)';
    chipBorder = '1px solid color-mix(in srgb, var(--p-accent) 40%, transparent)';
  } else if (isHovered) {
    chipBg = 'color-mix(in srgb, var(--p-accent) 14%, transparent)';
    chipBorder = '1px solid color-mix(in srgb, var(--p-accent) 30%, transparent)';
  }
...
function ContactCanvasSection(props: {
  ...
  return (
    ...
    >
      ...
      {/* Citizenship and eligibility row */}
      <div
        className="text-[10px] mt-1"
        style={{ color: 'var(--p-text-dim)' }}
        data-callout-anchor="contact-citizenship"
      >
        {contact.citizenship && <span>U.S. Citizen</span>}
        {contact.veteranStatus && contact.veteranStatus !== 'N/A' && (
          <span> | Veteran Preference: {contact.veteranStatus}</span>
        )}
        {!contact.citizenship && !contact.veteranStatus && (
          <span style={{ opacity: 0.4 }}>Citizenship / Eligibility not specified</span>
        )}
      </div>
    </div>
  );
        ...
        props.experience.map(function (exp) {
          ...
          return (
            <div key={exp.id} className="mb-3 relative" data-testid={'canvas-experience-' + exp.id}>
              {/* Remove job button — only visible in edit mode when section selected */}
              {props.editMode && props.isSelected && props.onRemoveJob && (
                <button
                  ...
                  className={'absolute -right-1 -top-0.5 flex items-center justify-center w-5 h-5 rounded-full outline-none focus-visible:ring-2 focus-visible:ring-inset ' + INTERACTIVE_HOVER_CLASS}
                  style={{
              ...
              <div className="flex items-baseline justify-between mb-0.5">
                ...
                {/* DATE RANGE — editable subfield */}
                {isExpFieldEditing(exp.id, 'dateRange') ? (
                  <div onClick={function (e: React.MouseEvent) { e.stopPropagation(); }}>
                    <InlineEditor
                      value={props.editingText}
                      onChange={props.onEditChange}
                      onSave={props.onEditSave}
                      ...
                      testId={'inline-editor-exp-dates-' + exp.id}
                    />
                  </div>
                ) : (
                  <ExperienceEditableField
                    value={(exp.startDate || exp.endDate) ? (exp.startDate + ' \u2013 ' + exp.endDate) : 'Add dates'}
                    isEmpty={!exp.startDate && !exp.endDate}
                    isSelected={props.isSelected}
                    onEditStart={function () { props.onFieldEditStart(exp.id, 'dateRange', (exp.startDate || '') + ' – ' + (exp.endDate || '')); }}
                    className="text-[10px]"
                    style={{ color: 'var(--p-text-muted)' }}
                    testId={'exp-field-dates-' + exp.id}
                    anchorId={'experience-dates-' + exp.id}
                  />
                )}
              </div>
...
// ---------------------------------------------------------------------------
// Document page break indicator — visual page flow separator
// ---------------------------------------------------------------------------

/**
 * Renders a visual page-break indicator when the document panel
 * content exceeds one letter-size page. Uses DOM measurement via
 * ResizeObserver to detect when the rendered document height crosses
 * page boundaries. This provides real document-flow behavior rather
 * than a static heuristic.
 *
 * PAGE SIZE REFERENCE: US Letter at 96dpi = 11in * 96 = 1056px.
 * After top/bottom padding (~80px total), the usable content area
 * is approximately 976px per page.
 *
 * The indicator appears as a dashed horizontal line with a "Page 2"
 * label, giving the user a clear signal that their content spans
 * multiple pages. The indicator repositions as content grows.
 */
function PageBreakIndicator(props: {
  documentPanelRef?: React.RefObject<HTMLDivElement | null>;
}) {
  const [pageCount, setPageCount] = useState(1);
  const PAGE_HEIGHT_PX = 976;

  useEffect(function () {
    const panel = props.documentPanelRef ? props.documentPanelRef.current : null;
    if (!panel) return;
    ...
    /* Use ResizeObserver to remeasure when content changes */
    ...
    return undefined;
  }, [props.documentPanelRef]);

  if (pageCount <= 1) return null;

  /* Render page break markers for each page boundary */
  ...
  for (let i = 1; i < pageCount; i++) {
    markers.push(
      <div
        ...
        style={{
          borderTop: '2px dashed color-mix(in srgb, var(--p-accent) 30%, transparent)',
          paddingTop: '4px',
        }}
        data-testid={'page-break-indicator-' + (i + 1)}
      >
        <span
          className="absolute -top-3 left-1/2 px-3 py-0.5 rounded text-[9px] font-semibold"
          style={{
...
function EmptyPlaceholder(props: { label: string; onClick?: () => void }) {
  ...
  return (
    <div
      className="py-3 text-center rounded cursor-pointer"
      style={{
        border: isHovered
