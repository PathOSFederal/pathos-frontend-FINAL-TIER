/**
 * ============================================================================
 * LIVE ADVISOR JOB SEARCH PROXY ROUTE
 * ============================================================================
 *
 * PURPOSE:
 * Proxy the frontend Job Search request through the PathOS backend so the
 * browser never calls USAJOBS directly. The backend remains the only surface
 * that talks to the official USAJOBS-backed search path.
 */
...
function extractBackendErrorMessage(
  ...
  if (
    ...
    return 'Live job search backend request failed: ' + typedPayload.detail.code;
  }
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          'The live job search route requires a structured search payload.',
      },
      { status: 400 }
    );
  }
  ...
  const response = await fetch(
    backend.config.baseUrl + '/api/v1/jobs/search',
    {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        error: extractBackendErrorMessage(
          payload,
          'The backend did not return live USAJOBS-backed search results.'
        ),
      },
