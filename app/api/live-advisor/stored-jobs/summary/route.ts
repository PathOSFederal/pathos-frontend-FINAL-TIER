/**
 * Proxy backend-owned Saved Jobs summary metrics through the same-origin frontend boundary.
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (
    ...
    return NextResponse.json(
      { error: 'The Saved Jobs summary route requires a structured payload.' },
      { status: 400 }
    );
  }

  const response = await fetch(
    backend.config.baseUrl + '/api/v1/advisor/intelligence/saved-jobs-summary',
    {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        error: extractBackendErrorMessage(
          payload,
          'The backend did not return Saved Jobs summary metrics.'
        ),
      },
