/**
 * ============================================================================
 * LIVE ADVISOR GENERIC EVALUATION PROXY ROUTE
 * ============================================================================
 *
 * PURPOSE:
 * Proxy one frontend advisor evaluation request to the backend deterministic
 * advisor endpoint. Job Search uses this route for selected-job evaluation.
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          'The live advisor evaluation route requires a structured advisor input payload.',
      },
      { status: 400 }
    );
  }
  ...
  const response = await fetch(
    backend.config.baseUrl + '/api/v1/advisor/evaluate',
    {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        error: extractBackendErrorMessage(
          payload,
          'The backend did not return a deterministic advisor evaluation.'
        ),
      },
