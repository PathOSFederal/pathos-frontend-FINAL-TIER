/**
 * Proxy the canonical Job Search batch intelligence payload through the backend.
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (
    ...
    return NextResponse.json(
      { error: 'The Job Search batch intelligence route requires a structured payload.' },
      { status: 400 }
    );
  }

  const response = await fetch(
    backend.config.baseUrl + '/api/v1/advisor/intelligence/job-search/batch',
    {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      {
        error: extractBackendErrorMessage(
          payload,
          'The backend did not return Job Search batch intelligence.'
        ),
      },
